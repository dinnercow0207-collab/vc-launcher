...
async function init() {
  ...
  if (mySuperAdmin) {
    document.getElementById('bonusSettingBox').style.display = 'block';
    await loadBonusSetting();
    document.getElementById('bonusAmountSaveBtn').addEventListener('click', saveBonusSetting);
  }
}
...
async function loadBonusSetting() {
  ...
  document.getElementById('bonusAmountInput').value = data.dailyBonusAmount;
}

async function saveBonusSetting() {
  const btn = document.getElementById('bonusAmountSaveBtn');
  const amount = document.getElementById('bonusAmountInput').value;
  btn.disabled = true;
  const res = await fetch('/api/admin/settings', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ dailyBonusAmount: amount }),
  });
  const data = await res.json();
  btn.disabled = false;
  if (!res.ok) {
...
async function loadUsers() {
  ...
  data.users.forEach((u) => {
    ...
    if (mySuperAdmin) {
      giftHtml = `
        <div class="gift-points-row">
          <input type="number" placeholder="±pt" data-gift-input="${u.id}">
          <button class="row-btn" data-action="gift" data-id="${u.id}">贈呈</button>
        </div>`;
    }
  ...
  tbody.querySelectorAll('[data-action]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (action === 'gift') {
        const input = tbody.querySelector(`[data-gift-input="${id}"]`);
        const amount = parseInt(input.value, 10);
        if (isNaN(amount) || amount === 0) {
          alert('贈呈するポイント数を入力してください(マイナスも可)');
          return;
        }
        btn.disabled = true;
        const res = await fetch(`/api/admin/users/${id}/gift-points`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const data = await res.json();
        btn.disabled = false;
        if (!res.ok) { alert(data.error || '贈呈に失敗しました'); return; }
        await loadUsers();
        return;
      }
