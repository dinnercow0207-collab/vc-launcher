...
async function ensureShopTables() {
  ...
  // 初期アイテムがまだ無ければ、お試し5個+完成記念1個を並べる
  const existing = await pool.query('SELECT COUNT(*) AS c FROM shop_items');
  if (Number(existing.rows[0].c) === 0) {
    const trialItems = [
      ['シンプルバッジ', 'ちょっとした特別感を演出する、控えめなバッジ', '🔹'],
      ['スターバッジ', 'きらりと光る星のバッジ', '⭐'],
      ['ハートバッジ', 'あたたかい気持ちを伝えるハートのバッジ', '💗'],
      ['フレイムバッジ', '燃えるような情熱を表すバッジ', '🔥'],
      ['クローバーバッジ', '見つけると嬉しい、幸運のバッジ', '🍀'],
    ];
    for (const [name, desc, badge] of trialItems) {
      await pool.query(
        'INSERT INTO shop_items (name, description, price, badge, tier) VALUES ($1, $2, 500, $3, $4)',
        [name, desc, badge, 'trial']
      );
    }
    await pool.query(
      `INSERT INTO shop_items (name, description, price, badge, tier)
       VALUES ('👑 完成記念クラウン', 'このサイトの完成を記念した、最上級の豪華なクラウンバッジ。持っているだけで一目置かれる特別な存在の証。', 3000, '👑', 'legendary')`
    );
  }
}
