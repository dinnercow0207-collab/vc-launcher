...
// 指定したVCチャンネルに今いるメンバー一覧を返す(同じユーザーの複数接続はまとめる)
function getVoiceOccupants(channelId) {
  const room = voiceRooms.get(channelId);
  if (!room) return [];
  const byUser = new Map();
  room.forEach((sid) => {
    const s = io.sockets.sockets.get(sid);
    if (!s || !s.request.session) return;
    const uid = s.request.session.userId;
    byUser.set(uid, {
      userId: uid,
      username: s.request.session.username,
      avatarUrl: s.data.avatarUrl || null,
      frameColor: s.data.frameColor || null,
    });
  });
...
io.on('connection', async (socket) => {
  ...
  socket.on('voice:join', (channelId) => {
    ...
    // 入る前から在室状況が分かるよう、全員に最新の在室メンバーを通知する
    io.emit('voice:presence', { channelId, users: getVoiceOccupants(channelId) });
  });
  ...
  // 各VCの在室状況を一括で返す(サーバーを開いたとき等)
  socket.on('voice:request-presence', () => {
    const all = {};
    voiceRooms.forEach((set, channelId) => {
      const occ = getVoiceOccupants(channelId);
      if (occ.length) all[channelId] = occ;
    });
    socket.emit('voice:presence-all', all);
  });
  ...
  function leaveVoice(socket) {
    ...
    // 退出後の最新の在室状況を全員に通知する(空になれば一覧から消える)
    io.emit('voice:presence', { channelId, users: getVoiceOccupants(channelId) });
