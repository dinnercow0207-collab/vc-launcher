...
app.get('/api/me', async (req, res) => {
  ...
  const result = await pool.query(`
    SELECT users.avatar_url, users.points, users.equipped_item_id, shop_items.badge
    FROM users
...
io.on('connection', async (socket) => {
  ...
  const userInfo = (await pool.query(`
    SELECT users.avatar_url, shop_items.badge
    FROM users
    LEFT JOIN shop_items ON shop_items.id = users.equipped_item_id
    WHERE users.id = $1
  `, [userId])).rows[0];
  const avatarUrl = userInfo?.avatar_url || null;
  const badge = userInfo?.badge || null;
  socket.data.badge = badge;
  ...
  socket.on('voice:join', (channelId) => {
    ...
    const existing = Array.from(room).map((id) => ({
      ...
      badge: io.sockets.sockets.get(id)?.data.badge || null,
    }));
    socket.emit('voice:existing-members', existing);

    room.add(socket.id);
    socket.join(`voice:${channelId}`);
    socket.to(`voice:${channelId}`).emit('voice:user-joined', { socketId: socket.id, username, badge });
  });
