...
app.get('/api/me', async (req, res) => {
  ...
  const result = await pool.query('SELECT avatar_url FROM users WHERE id = $1', [req.session.userId]);
  res.json({
    user: {
...
io.on('connection', async (socket) => {
  ...
  const userInfo = (await pool.query('SELECT avatar_url FROM users WHERE id = $1', [userId])).rows[0];
