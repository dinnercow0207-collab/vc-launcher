...
// ---- ボイスチェンジャー(ピッチシフト) ----
let voiceChangerEnabled = true;   // デフォルトでON
let pitchSemitones = 7;           // -12〜+12。+7は女性寄りの高め設定
let vcAudioCtx = null;            // ピッチシフト処理用のAudioContext
let vcProcessedStream = null;     // ピッチシフト後、相手に送るストリーム
...
// ---- VC在室状況(入る前から誰がいるか表示するため) ----
let voicePresence = {};             // channelId -> [{ userId, username, avatarUrl, frameColor }]
...
async function init() {
  ...
  bindUI();
  await loadServers();
  socket.emit('voice:request-presence'); // 各VCの在室状況を取得
...
async function selectServer(server) {
  ...
  // 他のサーバーを見ている間はVC画面を一旦隠す。
  // VCは裏で継続し、通話中バーの「通話画面に戻る」から確実に元のVC画面へ戻れるようにする。
  ...
  renderChannelLists();
  socket.emit('voice:request-presence'); // このサーバーのVC在室状況を最新化
...
function renderChannelLists() {
  ...
  channels.filter((c) => c.type === 'voice').forEach((c) => {
    const occupants = voicePresence[c.id] || [];
    const el = document.createElement('div');
    el.className = 'chan-item' + (currentVoiceChannel && currentVoiceChannel.id === c.id ? ' active' : '');
    const countLabel = occupants.length ? `<span class="icon">🔊 ${occupants.length}</span>` : `<span class="icon">🔊</span>`;
    el.innerHTML = `
      <span class="chan-label">${countLabel} ${escapeHtml(c.name)}</span>
      ${isMember ? `<button type="button" class="chan-menu-btn" data-id="${c.id}" title="操作">⋮</button>` : ''}`;
    el.querySelector('.chan-label').addEventListener('click', () => openVoiceChannel(c));
    voiceList.appendChild(el);

    // 入る前から、そのVCに今いるメンバーを一覧表示する
    if (occupants.length) {
      const occWrap = document.createElement('div');
      occWrap.className = 'vc-occupants';
      occWrap.innerHTML = occupants.map((u) => {
        const initial = (u.username || '?').slice(0, 1).toUpperCase();
        const frameClass = u.frameColor ? ' framed-avatar' : '';
        const frameStyle = u.frameColor ? ` style="--frame-color: ${escapeHtml(u.frameColor)};"` : '';
        const avatar = u.avatarUrl
          ? `<img class="vc-occ-avatar${frameClass}"${frameStyle} src="${escapeHtml(u.avatarUrl)}" alt="">`
          : `<span class="vc-occ-avatar vc-occ-fallback${frameClass}"${frameStyle}>${escapeHtml(initial)}</span>`;
        return `<span class="vc-occ" title="${escapeHtml(u.username)}">${avatar}<span class="vc-occ-name">${escapeHtml(u.username)}</span></span>`;
      }).join('');
      voiceList.appendChild(occWrap);
    }
  });
...
// ボイスチャンネル(VC) + 画面共有
// ===================================================================
// ---- 声質を明るく補正するフィルター(簡易フォルマント風味付け) ----
// 真のフォルマントシフトは複雑すぎるため、EQで「低音の太さを削り、
// 声の抜け・高域の明るさを足す」ことで女性寄りの声色に近づける現実的な代替。
let vcLowShelf = null;
let vcPresence = null;
let vcHighShelf = null;

function refreshVoiceColorEQ() {
  if (!vcLowShelf || !vcPresence || !vcHighShelf) return;
  const on = voiceChangerEnabled;
  vcLowShelf.gain.value = on ? -6 : 0;
  vcPresence.gain.value = on ? 5 : 0;
  vcHighShelf.gain.value = on ? 3 : 0;
}

// マイクの生ストリームを受け取り、ピッチシフト+声質補正後のストリームを返す。
// voiceChangerEnabled / pitchSemitones はスライダー操作でリアルタイムに反映される。
function buildProcessedStream(rawStream) {
  vcAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
  const source = vcAudioCtx.createMediaStreamSource(rawStream);

  vcLowShelf = vcAudioCtx.createBiquadFilter();
  vcLowShelf.type = 'lowshelf';
  vcLowShelf.frequency.value = 250;

  vcPresence = vcAudioCtx.createBiquadFilter();
  vcPresence.type = 'peaking';
  vcPresence.frequency.value = 3000;
  vcPresence.Q.value = 1;

  vcHighShelf = vcAudioCtx.createBiquadFilter();
  vcHighShelf.type = 'highshelf';
  vcHighShelf.frequency.value = 6000;
  ...
  const shifter = vcAudioCtx.createScriptProcessor(bufferSize, 1, 1);
  const ringSize = bufferSize * 4;
  ...
  shifter.onaudioprocess = (e) => {
    ...
    const ratio = voiceChangerEnabled ? Math.pow(2, pitchSemitones / 12) : 1;
    ...
    for (let i = 0; i < output.length; i++) {
      const idx = Math.floor(readIdx) % ringSize;
      output[i] = ring[idx];
      readIdx += ratio;
      if (readIdx >= ringSize) readIdx -= ringSize;
    }
  };

  source.connect(vcLowShelf);
  vcLowShelf.connect(vcPresence);
  vcPresence.connect(vcHighShelf);
  vcHighShelf.connect(shifter);
...
async function openVoiceChannel(channel) {
  ...
  try {
    ...
    vcProcessedStream = buildProcessedStream(localStream);
  } catch (e) {
  ...
  monitorSpeaking(localStream, 'tile-' + me.id + ':me');
...
function leaveVoice() {
  ...
  speakingAnimFrames.forEach((_, tileId) => stopMonitoringSpeaking(tileId));
  if (vcMonitorCtx) {
    vcMonitorCtx.close();
    vcMonitorCtx = null;
  }
...
function setupVoiceSocketHandlers() {
  // どのVCに今誰がいるかの一括受信(サーバー選択時などにリクエストする)
  socket.on('voice:presence-all', (all) => {
    voicePresence = all || {};
    renderChannelLists();
  });
  // 1つのVCの在室状況が変化したときの差分受信
  socket.on('voice:presence', ({ channelId, users }) => {
    if (users && users.length) {
      voicePresence[channelId] = users;
    } else {
      delete voicePresence[channelId];
    }
    renderChannelLists();
  });
  ...
  socket.on('voice:user-left', ({ socketId }) => {
    ...
    stopMonitoringSpeaking('tile-' + socketId);
...
function createPeerConnection(remoteSocketId, isInitiator) {
  ...
  if (vcProcessedStream) {
    vcProcessedStream.getTracks().forEach((track) => pc.addTrack(track, vcProcessedStream));
  } else if (localStream) {
  ...
  pc.ontrack = (e) => {
    ...
    if (e.track.kind === 'video') {
      ...
      attachRemoteAudio(e.track.id, stream, e.track, remoteSocketId);
    }
  };
...
// ---- 発話検知(喋ってる人のアイコンを光らせる) ----
let vcMonitorCtx = null;                 // 発話検知用の共有AudioContext
const speakingAnimFrames = new Map();    // tileId -> requestAnimationFrame id

function monitorSpeaking(stream, tileId) {
  if (!vcMonitorCtx) {
    vcMonitorCtx = new (window.AudioContext || window.webkitAudioContext)();
  }
  const source = vcMonitorCtx.createMediaStreamSource(stream);
  const analyser = vcMonitorCtx.createAnalyser();
  analyser.fftSize = 512;
  analyser.smoothingTimeConstant = 0.6;
  source.connect(analyser);
  const data = new Uint8Array(analyser.frequencyBinCount);
  const THRESHOLD = 12; // 平均音量がこれを超えたら「喋ってる」とみなす
  ...
  function tick() {
    ...
    speakingAnimFrames.set(tileId, requestAnimationFrame(tick));
  }
  tick();
}

function stopMonitoringSpeaking(tileId) {
  const frameId = speakingAnimFrames.get(tileId);
  if (frameId) cancelAnimationFrame(frameId);
  speakingAnimFrames.delete(tileId);
  const tile = document.getElementById(tileId);
  if (tile) tile.classList.remove('speaking');
}

function attachRemoteAudio(trackId, stream, track, socketId) {
  ...
  if (socketId) monitorSpeaking(stream, 'tile-' + socketId);
  if (track) {
    track.onended = () => {
      audioEl.remove();
      if (socketId) stopMonitoringSpeaking('tile-' + socketId);
    };
  }
}
...
function bindUI() {
  ...
  const vcToggleState = document.getElementById('vcToggleState');
  const vcPitchSlider = document.getElementById('vcPitchSlider');
  const vcPitchValue = document.getElementById('vcPitchValue');

  function refreshVcUI() {
    vcToggleState.textContent = voiceChangerEnabled ? 'ON' : 'OFF';
    vcToggleBtn.classList.toggle('off', !voiceChangerEnabled);
    vcPitchValue.textContent = (pitchSemitones > 0 ? '+' : '') + pitchSemitones;
    refreshVoiceColorEQ();
  }
  refreshVcUI();
  ...
  vcPitchSlider.addEventListener('input', (e) => {
