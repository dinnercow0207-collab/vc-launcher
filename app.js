...
function appendMessage(msg) {
  ...
  const frameStyle = msg.frame_color ? ` style="box-shadow: 0 0 0 3px ${escapeHtml(msg.frame_color)};"` : '';
  const avatarHtml = msg.avatar_url
    ? `<img class="avatar"${frameStyle} src="${escapeHtml(msg.avatar_url)}" alt="">`
    : `<div class="avatar"${frameStyle}>${escapeHtml(initial)}</div>`;
...
function addOrUpdateTile(id, username, isMe, frameColor, avatarUrl) {
  ...
  const frameStyle = frameColor ? ` style="box-shadow: 0 0 0 3px ${escapeHtml(frameColor)};"` : '';
  const avatarHtml = avatarUrl
    ? `<img class="avatar"${frameStyle} src="${escapeHtml(avatarUrl)}" alt="">`
    : `<div class="avatar"${frameStyle}>${escapeHtml(initial)}</div>`;
  tile.innerHTML = `
    ${avatarHtml}
...
function renderMyAvatar() {
  ...
  const frameShadow = me.frame_color ? `0 0 0 2px ${me.frame_color}` : '';
  if (me.avatar_url) {
    img.src = me.avatar_url;
    img.style.display = 'block';
    img.style.boxShadow = frameShadow;
    fallback.style.display = 'none';
  } else {
    img.style.display = 'none';
    fallback.style.display = 'flex';
    fallback.style.boxShadow = frameShadow;
...
function applyMyTheme() {
  const scroll = document.getElementById('chatScroll');
  if (!scroll) return;
  scroll.style.background = me.theme_bg || '';
}
  ...
  data.items.forEach((item) => {
    ...
    el.innerHTML = `
      <div class="shop-theme-preview" style="background:${item.theme_bg || 'var(--bg-2)'};">
        <span class="shop-theme-preview-ring" style="box-shadow: 0 0 0 3px ${item.frame_color};">${item.badge}</span>
      </div>
