...
function appendMessage(msg) {
  ...
  const badgeHtml = msg.badge ? `<span class="msg-badge">${msg.badge}</span>` : '';

  el.innerHTML = `
    ${avatarHtml}
    <div class="body">
      <div class="meta"><span class="name">${escapeHtml(msg.username)}${badgeHtml}</span>${time}</div>
...
async function openVoiceChannel(channel) {
  ...
  addOrUpdateTile(me.id + ':me', me.username, true, me.badge);
...
function setupVoiceSocketHandlers() {
  socket.on('voice:existing-members', (members) => {
    ...
    members.forEach(({ socketId, username, badge }) => {
      remoteMeta.set(socketId, { username, sharingScreen: false });
      createPeerConnection(socketId, true);
      addOrUpdateTile(socketId, username, false, badge);
    });
  });

  socket.on('voice:user-joined', ({ socketId, username, badge }) => {
    remoteMeta.set(socketId, { username, sharingScreen: false });
    addOrUpdateTile(socketId, username, false, badge);
...
// ---- 画面まわりのUI ----
function addOrUpdateTile(id, username, isMe, badge) {
  ...
  const badgeHtml = badge ? `<span class="msg-badge">${badge}</span>` : '';
  tile.innerHTML = `
    <div class="avatar">${escapeHtml(initial)}</div>
    <div class="name">${escapeHtml(username)}${badgeHtml}${isMe ? '（あなた）' : ''}</div>`;
...
async function openShop() {
  ...
  data.items.forEach((item) => {
    ...
    el.innerHTML = `
      <div class="shop-item-badge">${item.badge}</div>
  ...
  list.querySelectorAll('[data-action]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (action === 'buy') {
        ...
        me.badge = equippedItem?.badge || null;
